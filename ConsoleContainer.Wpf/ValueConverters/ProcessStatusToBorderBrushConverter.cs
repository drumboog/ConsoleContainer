...
namespace ConsoleContainer.Wpf.ValueConverters
{
    ...
    public sealed class ProcessStatusToBorderBrushConverter : IValueConverter
    {
        public object? Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            switch (state)
            {
                case ProcessState.Idle:
                    return new SolidColorBrush(Colors.Red) { Opacity = 0.4 };
                case ProcessState.Starting:
                    return new SolidColorBrush(Colors.Green) { Opacity = 0.5 };
                case ProcessState.Running:
                    return new SolidColorBrush(Colors.LightGreen) { Opacity = 0.5 };
                case ProcessState.Stopping:
                    return new SolidColorBrush(Colors.IndianRed) { Opacity = 0.4 };
                default:
